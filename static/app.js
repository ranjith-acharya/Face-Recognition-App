...
$(document).ready(function(){
    ...
    $('#recognize').submit(function(e){
        console.log('Recognize!', recognize_data);
        ...
        axios.post('/api/recognize', recog_form_data).then(function(response){
            console.log('We found a Match!', response.data);
            message = {type: 'success', message: 'We found a Match! : ' + response.data.user.name};
            update();
        }).catch(function(err){
            message = {type: 'danger', message: _.get(err, 'response.data.error.message', 'Unknown error')};
            update();
        });
        e.preventDefault();
    });
